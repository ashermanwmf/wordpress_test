...
function get_yp_editor_link(){
	...
	if($id === 0 || $id === null || is_tag() || is_category() || is_archive() || is_author() || is_search() || is_404()){
		...
		$href = add_query_arg(array('yellow_pencil' => 'true', 'yp_id' =>  $id,'href' => urlencode(get_permalink($id))),get_home_url());
	}
...
function yp_demo_editor_header(){
	...
	.theme-demo-options{display:none !important;}@media(max-width:992px){.yp-demo-link{display:none !important;}}</style><a href="'.get_yp_editor_link().'" class="yp-demo-link yp-live-editor-link">Live Editor</a>';
